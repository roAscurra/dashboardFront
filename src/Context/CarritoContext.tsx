...
interface CartContextType {
  cart: ArticuloManufacturadoShorDto[];
  addCarrito: (product: ArticuloManufacturadoShorDto) => void;
  removeCarrito: (product: ArticuloManufacturadoShorDto) => void;
  removeItemCarrito: (product:ArticuloManufacturadoShorDto) => void;
  limpiarCarrito: () => void;
  totalPedido?:number;
}
...
export function CarritoContextProvider({ children }: { children: ReactNode }){
    ...
    const[cart, setCart] = useState<ArticuloManufacturadoShorDto[]>([]);
    const[totalPedido, setTotalPedido] = useState<number>(0);

    const addCarrito = async (product:ArticuloManufacturadoShorDto) => {
        let existe:boolean = false
        cart.forEach(async (element:ArticuloManufacturadoShorDto) => {
    ...
    const removeCarrito = async (product: ArticuloManufacturadoShorDto) => {
        await setCart(prevCart => prevCart.filter(item => item.id !== product.id))
    };

    const removeItemCarrito = async (product: ArticuloManufacturadoShorDto) => {
